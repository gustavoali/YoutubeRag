#!/bin/bash
# ============================================
# YoutubeRag Database Seeding Script (Bash)
# ============================================
...
DATABASE="${DATABASE:-youtube_rag_db}"
USER="${USER:-root}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  -e, --environment ENV    Environment (Development, Testing) [default: Development]"
            echo "  -h, --host HOST          MySQL host [default: localhost]"
            echo "  -p, --port PORT          MySQL port [default: 3306]"
            echo "  -d, --database DB        Database name [default: youtube_rag_db]"
            echo "  -u, --user USER          MySQL user [default: root]"
            echo "  -w, --password PASS      MySQL password [default: rootpassword]"
            echo "  -c, --clean              Clean existing test data first"
...
# Header
log_header "YoutubeRag Database Seeding ($ENVIRONMENT)"

# Validate environment
if [ "$ENVIRONMENT" = "Production" ]; then
    log_error "Cannot seed Production database with test data!"
    log_info "Use -e 'Development' or 'Testing' instead"
    exit 1
fi

# Test database connection
log_info "Testing database connection..."
if docker exec youtube-rag-mysql mysqladmin ping -h "$MYSQL_HOST" -u "$USER" -p"$PASSWORD" >/dev/null 2>&1; then
    log_success "Database connection successful"
else
    log_error "Failed to connect to MySQL database"
    log_error "Ensure MySQL container is running: docker ps | grep youtube-rag-mysql"
    exit 1
fi

# Clean existing test data if requested
if [ "$CLEAN_FIRST" = true ]; then
    log_warning "Cleaning existing test data..."

    read -r -d '' CLEAN_SQL << 'EOF' || true
...
EOF
    ...
    echo "$CLEAN_SQL" | docker exec -i youtube-rag-mysql mysql -u "$USER" -p"$PASSWORD" "$DATABASE"

    if [ $? -eq 0 ]; then
        log_success "Test data cleaned"
    else
        log_warning "Clean operation had warnings (this may be normal)"
    fi
fi
...
echo "$SEED_SQL" | docker exec -i youtube-rag-mysql mysql -u "$USER" -p"$PASSWORD" "$DATABASE"
...
echo "  • admin@youtuberag.com (password: Admin123!)"
echo "  • user1@test.example.com (password: Test123!)"
echo "  • user2@test.example.com (password: Test123!)"
echo -e "${NC}  • inactive@test.example.com (password: Test123!) - Inactive"
echo ""

log_success "Test videos created:"
echo "  • 5 sample videos with different statuses"
echo "  • Statuses: Completed, Processing, Pending, Failed, Long Video"
echo ""

log_success "Test jobs created:"
echo "  • 5 sample jobs in various states"
echo "  • Types: Transcription, VideoProcessing, Embedding"
echo "  • Statuses: Completed, Running, Pending, Failed"
echo ""

log_success "Additional data:"
echo "  • 5 transcript segments for completed video"
echo "  • 3 user notifications"
echo ""

log_info "You can now test the application with this seed data"
log_info "To clean and re-seed: ./seed-database.sh --clean"
echo ""
